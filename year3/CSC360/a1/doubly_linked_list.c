...
struct node* concat(struct node* ref1, struct node* ref2) {
    /* Your code here */
    struct node* end_node = end(ref1);
    struct node* start = begin(ref2);
    end_node->next = start;
    start->prev = end_node;
...
int distance(struct node* ref1, struct node* ref2) {
    ...
    int dist = 0;
    /* Your code here */
    struct node* fwd = ref1;
    struct node* rvs = ref1;
    int fwd_flag = 1;
    int rvs_flag = 1;
    int i, j;
    for (i = 0, j = 0; ; i++, j--) {
...
struct node* rotate_left(struct node* ref, int n) {
    ...
    /* Your code here */
    for (int i = 0; i < n; i++) {
        struct node* new_end = push_back(ref, -1);
        swap(new_end, head);
        head = pop_front(ref);
    }
